...
type StatusVariant = VariantProps<typeof statusBadgeVariants>['status'];
...
export function StatusBadge({ className, status, ...props }: StatusBadgeProps) {
  ...
  const variantStatus = status as StatusVariant;

  return (
    <Badge
      className={cn(statusBadgeVariants({ status: variantStatus }), 'font-semibold', className)}
      {...props}
    >
      {statusText}
    </Badge>
